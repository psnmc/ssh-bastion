...
func (l *LogChannel) SyncToFile(remote_name string, remote_user string) error {
	var err error

	remote_name = sanitize.Name(remote_name)
	remote_user = sanitize.Name(remote_user)

	filepath := fmt.Sprintf("%s/%s/%d/%d", config.Global.LogPath, remote_user, l.StartTime.Year(), l.StartTime.Month())
